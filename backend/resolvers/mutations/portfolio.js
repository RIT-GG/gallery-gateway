...
export function createPortfolio (_, args, context) {
...
export function deletePortfolio (_, args, context) {
  // Only admins can delete portfolios
  if (context.authType !== ADMIN) {
    throw new UserError('Permission Denied')
  }
  return db.transaction(transaction =>
    Portfolio.findByPk(args.id, {transaction})
      .then(portfolio => {
        if (!portfolio) {
          // If the portfolio doesn't exist, then no need to delete anything
          return Promise.resolve(true)
        }
        // Otherwise, get all the entries with attachments so we can delete their files
        return Entry.findAll({where: {portfolioId: args.id}, transaction})
          .then(entries => {
          ...
          .then(() => portfolio.destroy({transaction}))
      })
  )
}
